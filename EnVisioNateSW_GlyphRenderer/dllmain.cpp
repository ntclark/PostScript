...
#include <shlobj.h>
    ...
    *ppObject = NULL;
    if ( ! ( CLSID_GlyphRenderer == rclsid  ) )
        return CLASS_E_CLASSNOTAVAILABLE;
    return objectFactory.QueryInterface(riid,ppObject);
    }


    const char *OBJECT_NAME = "EnVisioNateSW.GlyphRenderer";
    const char *OBJECT_NAME_V = "EnVisioNateSW.GlyphRenderer.1";
    const char *OBJECT_VERSION = "1.0";
    const char *OBJECT_DESCRIPTION = "GlyphRenderer to render TrueType font text. Learned how from FreeType, then left that in the dust where it belongs";
    ...
    memcpy(&OBJECT_CLSID,&CLSID_GlyphRenderer,sizeof(GUID));
    memcpy(&OBJECT_LIBID,&LIBID_GlyphRenderer,sizeof(GUID));
    ...
    *ppv = NULL; 
    ...
    GlyphRenderer *pef = new GlyphRenderer(punkOuter);
