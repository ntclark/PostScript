...
HBITMAP getSampleImage(HDC hdc);
    ...
    //
    // The units of the data that you are working with 
    // are called PAGE SPACE, in this case 0 <-> 100
    //
    ...
    // The negative value for eM22 is because windows
    // coordinates (DEVICE SPACE) are positive
    // in the y-direction from top to bottom.
    //
    ...
    // it translates that flipped value 
    // to the appropriate y value in device space
    //
    ...
    // desired rendering box, therefore, add TOP margin
    // to the height value.
    ...
    pIRenderer -> put_ToDeviceTransform((UINT_PTR)&xFormToPixels);
    ...
    while ( *p ) {
        pIFont -> RenderGlyph((unsigned short)*p,NULL,(UINT_PTR)&xFormToPoints,&startPoint,&endPoint,NULL,NULL);
        startPoint = endPoint;
        p++;
    }
