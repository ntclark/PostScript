...
#include "Renderer.h"
  ...
  Renderer::_IEnumConnectionPoints::_IEnumConnectionPoints(Renderer *pp,_IConnectionPoint **cp,int connectionPointCount) : 
  ...
  Renderer::_IEnumConnectionPoints::~_IEnumConnectionPoints() { 
  for ( int k = 0; k < cpCount; k++ )
  ...
  HRESULT Renderer::_IEnumConnectionPoints::QueryInterface(REFIID riid,void **ppv) {
  return pParent -> QueryInterface(riid,ppv);
  }
  unsigned long __stdcall Renderer::_IEnumConnectionPoints::AddRef() {
  return pParent -> AddRef();
  }
  unsigned long __stdcall Renderer::_IEnumConnectionPoints::Release() {
  return pParent -> Release();
  }


  HRESULT Renderer::_IEnumConnectionPoints::Next(ULONG countRequested,IConnectionPoint **rgpcn,ULONG *pcFetched) {
  ULONG found;
  ...
  HRESULT Renderer::_IEnumConnectionPoints::Skip(ULONG cSkip) {
  ...
  HRESULT Renderer::_IEnumConnectionPoints::Reset() {
  enumeratorIndex = 0;
  return S_OK;
  }

  HRESULT Renderer::_IEnumConnectionPoints::Clone(IEnumConnectionPoints **ppEnum) {
